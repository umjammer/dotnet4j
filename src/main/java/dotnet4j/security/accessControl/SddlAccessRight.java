...
class SddlAccessRight {
    ...
    public static SddlAccessRight lookupByName(String s) {
        for (SddlAccessRight right : rights) {
            if (Utilities.equals(right.Name, s))
                return right;
        }
    ...
    public static SddlAccessRight[] decompose(int mask) {
        ...
        int accountedBits = 0;
        for (SddlAccessRight right : rights) {
            if ((mask & right.Value) == right.Value && (accountedBits | right.Value) != accountedBits) {

                if (foundType == 0)
                    foundType = right.ObjectType;

                if (right.ObjectType != 0 && foundType != right.ObjectType)
                    return null;
    ...
    private static final SddlAccessRight[] rights = new SddlAccessRight[] {
        ...
        new SddlAccessRight() {{ Name = "GR"; Value = 0x80000000;}},
