...
class ExecutionContext {
    ...
    public static <T> void run(ExecutionContext executionContext, Consumer<T> callback, T state) {
    ...
    static void restore(Thread currentThread, ExecutionContext executionContext) {
        ...
        // For the purposes of dealing with context change, treat this as the
        // default EC
        if (executionContext == null) {
            executionContext = DEFAULT;
        }
        ...
        if (current.localChangeNotifications != previous.localChangeNotifications) {
            try {
                for (AsyncLocal<?> local : current.localChangeNotifications) {
                    // If the local has a value in the previous context, we
                    // already fired the event for that local
                    // in the code above.
                    Object previousValue = previous.localValues.get(local);
...
//                        ex);
    ...
    static <T> T getLocalValue(AsyncLocal<? extends T> local) {
        ...
        T result = current.localValues.get(local);
        return result;
    }
    ...
    static <T> void setLocalValue(AsyncLocal<T> local, T newValue, boolean needChangeNotifications) {
        ...
        //
        // Either copy the change notification array, or create a new one,
        // depending on whether we need to add a new item.
        //
        AsyncLocal<?>[] newChangeNotifications = current.localChangeNotifications;
        if (needChangeNotifications) {
            if (hadPreviousValue) {
                assert Arrays.asList(newChangeNotifications).contains(local);
            } else {
    ...
    public final ExecutionContext createCopy() {
        // since CoreCLR's ExecutionContext is immutable, we don't need to
        // create copies.
        return this;
    }
    ...
    public static Runnable wrap(Runnable runnable) {
        ...
        return () -> run(executionContext, Runnable::run, runnable);
    }
