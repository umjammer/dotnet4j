...
abstract class AsyncLocalValueMap {
    ...
    private static final class EmptyAsyncLocalValueMap extends AsyncLocalValueMap {
        ...
        public <T> AsyncLocalValueMap put(AsyncLocal<T> key, T value) {
            // If the value isn't null, then create a new one-element map to
            // store
            // the key/value pair. If it is null, then we're still empty.
            return value != null ? new OneElementAsyncLocalValueMap(key, value) : this;
        }
    ...
    private static final class OneElementAsyncLocalValueMap extends AsyncLocalValueMap {
        ...
        public <T> AsyncLocalValueMap put(AsyncLocal<T> key, T value) {
            ...
            if (value != null) {
                // The value is non-null. If the key matches one already
                // contained in this map,
                // then create a new one-element map with the updated value,
                // otherwise create
                ...
                // Otherwise, there's nothing to add or remove, so just return
                // this map.
    ...
    private static final class TwoElementAsyncLocalValueMap extends AsyncLocalValueMap {
        ...
        public <T> AsyncLocalValueMap put(AsyncLocal<T> key, T value) {
            ...
            if (value != null) {
                // The value is non-null. If the key matches one already
                // contained in this map,
                // then create a new two-element map with the updated value,
                // otherwise create
    ...
    private static final class ThreeElementAsyncLocalValueMap extends AsyncLocalValueMap {
        ...
        public <T> AsyncLocalValueMap put(AsyncLocal<T> key, T value) {
            ...
            if (value != null) {
                ...
                // The key doesn't exist in this map, so upgrade to a multi map
                // that contains
                ...
                // The value is null. If the key exists in this map, remove it
                // by downgrading to a two-element map
                // without the key. Otherwise, there's nothing to add or remove,
                // so just return this map.
    ...
    private static final class MultiElementAsyncLocalValueMap extends AsyncLocalValueMap {
        ...
        public <T> AsyncLocalValueMap put(AsyncLocal<T> key, T value) {
            ...
            for (int i = 0; i < keys.length; i++) {
                if (key == keys[i]) {
                    // The key is in the map. If the value isn't null, then
                    // create a new map of the same
                    // size that has all of the same pairs, with this new
                    // key/value pair overwriting the old.
                    if (value != null) {
                        ...
                        // The value is null, and we only have four elements,
                        // one of which we're removing,
                        ...
                        // Create a new one and copy all of the elements from
                        // this one, except the one to be removed.
            ...
            // If the value is null, then we can simply return this same map, as
            // there's nothing to add or remove.
            if (value == null) {
                return this;
            }

            // We need to create a new map that has the additional key/value
            // pair.
    ...
    private static final class ManyElementAsyncLocalValueMap extends AsyncLocalValueMap {
        ...
        public <T> AsyncLocalValueMap put(AsyncLocal<T> key, T value) {
            ...
            // If the value being set exists, create a new many map, copy all of
            // the elements from this one,
            // and then store the new key/value pair into it. This is the most
            // common case.
            if (value != null) {
            ...
            // Otherwise, the value is null, which means null is being stored
            // into an AsyncLocal.Value.
            // Since there's no observable difference at the API level between
            // storing null and the key
            // not existing at all, we can downgrade to a smaller map rather
            // than storing null.
            // If the key is contained in this map, we're going to create a new
            // map that's one pair smaller.
            if (containsKey) {
                // If the new count would be within range of a multi map instead
                // of a many map,
                // downgrade to the many map, which uses less memory and is
                // faster to access.
                // Otherwise, just create a new many map that's missing this
                // key.
            ...
            // We were storing null, but the key wasn't in the map, so there's
            // nothing to change.
